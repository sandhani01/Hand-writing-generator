...
def preprocess_image(image, skip_open=False):
...
def extract(
    ...
    thresh = preprocess_image(image, skip_open=skip_morph)
    ...
    for index, (char, roi, _, _, row, col) in enumerate(ordered_cells):
        ...
        if clean_symbols and group == "symbol":
            roi = clean_small_components(roi, min_area=4, ratio=0.08)
            roi = crop_to_content(roi, pad=2)
