...
def list_datasets(current_user: User = Depends(get_current_user)) -> DatasetListResponse:
    items = list_user_datasets(current_user.id)
...
def update_dataset_name(
    ...
    dataset = rename_dataset(current_user.id, dataset_id, payload.display_name)
    return DatasetResponse.model_validate(dataset)
...
def remove_dataset(
    ...
    dataset = delete_dataset(current_user.id, dataset_id)
